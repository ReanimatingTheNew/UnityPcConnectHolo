...
public class TcpClientUwpAndUnity : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        {
            Connect(GameSettings.Instance.My_Partner_IP, GameSettings.Instance.My_Partner_Listen_Port);//should use same port on server as client    
        }
        else {
         ...
         if (GameSettings.Instance != null)
        {
                ConnectUWP(GameSettings.Instance.My_Partner_IP, GameSettings.Instance.My_Partner_Listen_Port);//should use same port on server as client    
        }
        else {
            Debug.Log("no settings found , assuming server is on jalt");
            Connect("192.168.1.3","50103");
